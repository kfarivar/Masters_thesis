''' This is the code from pl_bolts. they only supported Resnet50 with some minor changes I made
(changin where 'hidden_mlp' was used as feature dimension to 512) it also works with Resnet18.
'''
...
class SimCLR(LightningModule):
    def __init__(
        self,
        gpus: int=1,#kiya
        num_samples: int=1, # kiya
        batch_size: int=256, #kiya
        dataset: str = 'cifar10', # kiya
        num_nodes: int = 1,
        arch: str = "resnet18", # default modified by kiya
        hidden_mlp: int = 2048,
        feat_dim: int = 128,
        warmup_epochs: int = 10,
        max_epochs: int = 100,
        temperature: float = 0.1,
        first_conv: bool = False, # kiya
        maxpool1: bool = False, # kiya
        optimizer: str = "adam",
...
def cli_main():
    ...
    print()
    print('MY simCLR ARCH:')
    for name, module in model.named_children():
        print()
        print(name)
        print(module)
    print('MY only encoder:')
